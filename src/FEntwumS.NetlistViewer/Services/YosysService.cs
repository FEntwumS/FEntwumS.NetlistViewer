...
public class YosysService : IYosysService
{
    ...
    public async Task<bool> LoadVerilogAsync(IProjectFile file)
    {
        ...
        [
            "-p",
            $"read_verilog -sv -nooverwrite \"{string.Join("\" \"", verilogFileList)}\" \"{string.Join("\" \"", systemVerilogFileList)}\"; scratchpad -set flatten.separator \";\"; {_fpgaBbService.getBbCommand()} hierarchy -check -top {top}; proc; memory -nomap; flatten -scopename; write_json -compat-int {top}.json"
        ];
