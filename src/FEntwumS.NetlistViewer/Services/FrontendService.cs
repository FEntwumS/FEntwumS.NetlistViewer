...
public class FrontendService : IFrontendService
{
	...
	{
		_settingsService.GetSettingObservable<string>(FentwumSNetlistViewerSettingsHelper.BackendAddressKey).Subscribe(
			x =>
			{
				if (IsAddressValid(x))
				{
		...
		_settingsService.GetSettingObservable<string>(FentwumSNetlistViewerSettingsHelper.BackendRequestTimeoutKey)
			.Subscribe(x =>
			{
				try
				{
		...
		_settingsService.GetSettingObservable<string>(FentwumSNetlistViewerSettingsHelper.EntityFontSizeKey).Subscribe(
			x =>
			{
				try
				{
		...
		_settingsService.GetSettingObservable<string>(FentwumSNetlistViewerSettingsHelper.CellFontSizeKey).Subscribe(
			x =>
			{
				try
				{
		...
		_settingsService.GetSettingObservable<string>(FentwumSNetlistViewerSettingsHelper.EdgeFontSizeKey).Subscribe(
			x =>
			{
				try
				{
		...
		_settingsService.GetSettingObservable<string>(FentwumSNetlistViewerSettingsHelper.PortFontSizeKey).Subscribe(
			x =>
			{
				try
				{
		...
		_settingsService.GetSettingObservable<string>(FentwumSNetlistViewerSettingsHelper.JavaArgsKey)
			.Subscribe(x => _extraJarArgs = x);
	...
	private async Task<(bool success, bool needsRestart)> InstallDependenciesAsync()
	{
		...
		[
			("OneWare.GhdlExtension", new Version(1, 0, 0), []),
				...
				(FEntwumSNetlistReaderFrontendModule.NetlistViewerBackendPackage.Id!, new Version(0, 12, 5), []),
		...
		NetlistType netlistType, string topEntityName)
	{
		...
		ApplicationProcess proc =
			_applicationStateService.AddState($"Visualizing {netlistLanguageString} netlist", AppState.Loading);
	...
	private async Task<bool> ShowHierarchyAsync(IProjectFile netlistFile)
	{
		...
		(HierarchySideBarElement? elem, List<HierarchyViewElement>? elements) =
			await _hierarchyJsonParser.LoadHierarchyAsync(await resp.Content.ReadAsStreamAsync(), combinedHash);
