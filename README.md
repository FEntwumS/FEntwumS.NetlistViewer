...
| Category             | Dependency     | Minimum version | Incompatible versions |
|----------------------|----------------|-----------------|-----------------------|
| Plugins > Simulators | GHDL Extension | 1.0.0           | none                  |
| Binaries             | GHDL           | 5.0.1           | 5.1.1                 |
| Binaries             | OSS CAD Suite  | 2025.01.22      | none                  |

The plugin also ships some dependencies that must be manually installed after the plugin has been installed. Detailed
innstallation instructions are available in the [Installation](#installation) section. The plugin will also prompt you
...
The backend is capable of displaying most JSON-netlist generated by yosys. There is only one requirement: All modules
referenced in the design need to have a module definition in the netlist. If your design uses device-specific
primitives, you can enable automatic loading of the relevant definitions both in the global and project-specific
settings. The project-specific settings take precedence over the global settings.

For the global settings:
1. Go to Settings > Netlist Viewer
2. Scroll down to the "FPGA" section or use the menu on the left side
3. Set the manufacturer and device family hust like you would configure the correct synthesis pass in yosys

For the project-specific settings:
1. Open the project settings by right-clicking the project root
2. Go to "Other"
3. 3. Set the manufacturer and device family hust like you would configure the correct synthesis pass in yosys

If your design uses third-party modules not part of your OneWare project, you will need to include a file that defines
the interface of the module (it does not need to implement the behavior).

### Viewing post-synthesis netlists

If you have synthesized your design, you can view the synthesized netlist by right-clicking the file containing the
toplevel entity and selecting the `Vie Post-Synthesis Netlist for ...` option.
